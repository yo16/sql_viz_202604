...
test('initial state is compact: edge.target points to QueryBox (bd-boe)', () => {
  // bd-boe: syncFromLineage 後は全ノード compact → edge は QueryBox 本体を指す
  reset();
  ...
  assert(edge.target === 'b',
    'expected target=b (compact), got: ' + edge.target);
...
test('toggling to detail: edge.target points to FROM clauseBox', () => {
  ...
  assert(edge.target === 'b__clause__FROM',
    'expected target=b__clause__FROM after detail toggle, got: ' + edge.target);
});
...
test('toggling back to compact: edge.target reverts to QueryBox', () => {
  ...
  const edge = findTableDepEdge();
  assert(edge.target === 'b',
    'expected target=b after compact re-toggle, got: ' + edge.target);
});
